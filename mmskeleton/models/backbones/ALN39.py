...
add conv on M but conv's parameter can not be bp
融合为0.5 0.5 并且spatial 
为了测试融合和batch没错 
...
class ST_GCN_ALN39(nn.Module):
    ...
    def __init__(self,
        ...
        # self.M_weight_no_grad = self.M_weight.detach()
    ...
    def forward(self, x):
        ...
        A = self.A.view(1,3,25,25).repeat(N*M,1,1,1)
        # index = 0
        # for i in range(25):
        #     for j in range(i + 1):
        #        for n in range(N*M):
